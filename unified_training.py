...
def convert_to_yolo():
    ...
    # 运行转换脚本
    try:
        cmd = [sys.executable, "labelme2yolo.py", 
               "--input_dir", input_dir, 
...
def check_dataset():
    ...
    # 检查多个数据集选项
    datasets = {
        "COCO128": "data/coco128_custom.yaml",
        "LabelMe练习集": "datasets/yolo_practice/dataset.yaml"
    }
...
def start_training():
    ...
    cmd = [
        ...
        "--img", "640",
        "--epochs", "30",  # 适中的训练轮次
        "--batch-size", "8",  # 较小的批次大小
        "--project", project_name,  # 自定义项目名称
        "--name", exp_name  # 自定义实验名称
    ]
